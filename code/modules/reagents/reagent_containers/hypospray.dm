...
/obj/item/reagent_containers/hypospray/attack(mob/living/M, mob/user)
	...
	if (allow == INJECTION_PORT)
		...
		if (!do_after(user, INJECTION_PORT_DELAY, M, DO_DEFAULT | DO_BOTH_UNIQUE_ACT, do_skill = SKILL_MEDICAL, timer_flags = DO_TIMER_FLAG_USER_SKILL))
			return
	...
	if(user != M && !M.incapacitated() && time) // you're injecting someone else who is concious, so apply the device's intrisic delay
		...
		if (!do_after(user, time, M, DO_PUBLIC_UNIQUE, do_skill = SKILL_MEDICAL, timer_flags = DO_TIMER_FLAG_USER_SKILL))
			return
