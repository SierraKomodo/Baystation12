...
/mob/living/exosuit/proc/install_system(var/obj/item/system, var/system_hardpoint, var/mob/user)
	...
	if(user)
		...
		if(!do_after(user, DO_TIMER_QUICK, src, DO_PUBLIC_UNIQUE, do_skill = SKILL_DEVICES, timer_flag = DO_TIMER_FLAG_USER_SKILL) || user.get_active_hand() != system)
			return FALSE
...
/mob/living/exosuit/proc/remove_system(var/system_hardpoint, var/mob/user, var/force)
	...
	if(user)
		...
		if(!do_after(user, DO_TIMER_QUICK, src, DO_PUBLIC_PROGRESS, do_skill = SKILL_DEVICES, timer_flag = DO_TIMER_FLAG_USER_SKILL) || hardpoints[system_hardpoint] != system)
			return FALSE
