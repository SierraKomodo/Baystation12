...
/obj/item/mech_component/proc/repair_brute_generic(var/obj/item/weldingtool/WT, var/mob/user)
	...
	if(WT.remove_fuel((SKILL_MAX + 1) - user.get_skill_value(SKILL_CONSTRUCTION), user))
		...
		if (do_after(user, DO_TIMER_MIN, src, DO_PUBLIC_UNIQUE, do_skill = SKILL_DEVICES, timer_flags = DO_TIMER_FLAG_USER_SKILL) && brute_damage)
...
/obj/item/mech_component/proc/repair_burn_generic(var/obj/item/stack/cable_coil/CC, var/mob/user)
	...
	if (do_after(user, DO_TIMER_MIN, src, DO_PUBLIC_UNIQUE, do_skill = SKILL_DEVICES, timer_flags = DO_TIMER_FLAG_USER_SKILL) && burn_damage)
