...
/obj/structure/cable/attackby(obj/item/W, mob/user)
	if(isWirecutter(W))
		...
		if(W.force < 5)
			...
			delay_holder = DO_TIMER_MEDIUM
		else
			visible_message("<span class='warning'>[user] begins to cut through the cable with \the [W].</span>")
			delay_holder = DO_TIMER_QUICK

		if (do_after(user, delay_holder, src, DO_PUBLIC_UNIQUE, do_skill = SKILL_ELECTRICAL, timer_flags = DO_TIMER_FLAG_USER_SKILL))
			cut_wire(W, user)
